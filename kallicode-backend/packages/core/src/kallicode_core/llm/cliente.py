...
async def ejecutar_paso(
    paso_id: str, entrada: dict[str, Any], tier_minimo: Tier | None = None
) -> ResultadoPaso:
    ...
    while True:
        ...
        while True:  # reintentos dentro del tier
            ...
            try:
                ...
                intentos.append(
                    {"tier": tier.value, "modelo": cfg.modelo, "resultado": "proveedor_fallo"}
                )
            ...
            try:
                ...
                feedback = str(e)[:800]
                log.warning(
                    "llm.esquema_invalido", paso=paso_id, tier=tier.value, intento=fallos_esquema
                )
                intentos.append(
                    {"tier": tier.value, "modelo": cfg.modelo, "resultado": "esquema_invalido"}
                )
            ...
            if (
                ...
                and confianza < s.llm_umbral_confianza
                and pl["escalable"]
                and siguiente_tier(tier) is not None
            ):
                log.info("llm.baja_confianza", paso=paso_id, tier=tier.value, confianza=confianza)
                intentos.append(
                    {
                        "tier": tier.value,
                        "modelo": cfg.modelo,
            ...
            validacion = (
                "esquema_valido"
                if not intentos
                else ("escalado_tier" if intentos[-1]["tier"] != tier.value else "reintento")
            )
            log.info(
                "llm.paso_ok",
                paso=paso_id,
        ...
        if prox is None:
            log.error(
                "llm.escalado_humano", paso=paso_id, ultimo_tier=tier.value, intentos=len(intentos)
            )
