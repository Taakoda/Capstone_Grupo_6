...
EVENTOS = (
    ...
    "organizacion_modificada",
    "conexion_creada",
    "conexion_modificada",
    "conexion_eliminada",
    "llave_modelo_registrada",
...
async def verificar_cadena(db: AsyncSession, tenant_id: str, desde=None, hasta=None) -> dict:
    ...
    if fila and not fila["integra"]:
        log.critical(
            "auditoria.cadena_rota", tenant=tenant_id, primer_evento=fila["primer_evento_roto"]
        )
