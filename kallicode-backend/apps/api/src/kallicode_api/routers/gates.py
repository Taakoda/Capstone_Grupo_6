...
async def _ticket_y_job(db, tenant_id: str, numero: str) -> tuple[dict, dict | None]:
    ...
    job = await uno(
        db,
        """SELECT * FROM core.jobs WHERE ticket_id=:tk
...
async def pendientes(
    gate: int | None = None, usuario: UsuarioActual = Depends(requiere_rol(*_ROLES_APROBADOR))
) -> dict:
    ...
    que = {
        1: "Aprobar spec y alcance",
        2: "Validar evidencia en staging",
    ...
    items = [
        {
            ...
            "titulo": f["titulo"],
            "gate": f["gate"],
            "que_se_aprueba": que[f["gate"]],
            "esperando_desde": f["actualizado_en"],
        }
        for f in filas
...
async def estado_gates(numero: str, usuario: UsuarioActual = Depends(usuario_actual)) -> dict:
    ...
    async with sesion_de(usuario) as db:
        ...
        firmas = await todos(
            ...
            SELECT g.gate, g.accion, u.nombre AS actor, g.creado_en AS fecha,
             ...
             WHERE g.ticket_id = :tk ORDER BY g.gate, g.creado_en""",
            {"tk": t["id"]},
        )
    gates = []
    for n in (1, 2, 3):
        aprobada = next((f for f in firmas if f["gate"] == n and f["accion"] == "aprobado"), None)
        if aprobada:
            gates.append(
                {
                    "gate": n,
                    "estado": "firmado",
                    "actor": aprobada["actor"],
                    "fecha": aprobada["fecha"],
            ...
            gates.append(
                {"gate": n, "estado": "pendiente" if t["gate_pendiente"] == n else "no_alcanzado"}
            )
    return {"gates": gates}
...
async def aprobar(
    ...
    gate: int,
    datos: FirmaIn,
    usuario: UsuarioActual = Depends(requiere_rol(*_ROLES_APROBADOR)),
) -> dict:
    ...
    if not puede_firmar(usuario.rol, gate):
        raise AppError(
            "FIRMA_NO_AUTORIZADA",
            403,
            "El Gate 3 requiere la firma de un arquitecto."
            if gate == 3
    ...
    async with sesion_de(usuario) as db:
        ...
        if t["gate_pendiente"] != gate or not job:
            raise AppError("GATE_NO_ACTIVO", 409, f"Este ticket no está esperando el Gate {gate}.")
        spec = await uno(
            db,
        ...
        if gate == 1:
            ...
            alts = await todos(
                db, "SELECT * FROM core.spec_alternatives WHERE spec_id=:s", {"s": spec["id"]}
            )
            if len(alts) > 1 and not datos.alternativa_id:
                raise AppError(
                    "ALTERNATIVA_REQUERIDA",
                    422,
                    "Debes elegir una alternativa de la spec para aprobar el diseño.",
                )
            if datos.alternativa_id:
                ...
                if ok.rowcount == 0:
                    raise AppError(
                        "VALIDACION_ENTRADA", 422, "La alternativa indicada no existe en la spec."
                    )
            await db.execute(
                text("UPDATE core.specs SET estado='aprobada' WHERE id=:s"), {"s": spec["id"]}
            )
        elif gate == 2:
            run = await uno(
                db,
                """SELECT * FROM core.qa_runs WHERE ticket_id=:tk
                                    ORDER BY corrida DESC LIMIT 1""",
                {"tk": t["id"]},
            )
            if not run:
                raise AppError(
                    "PRECONDICION_GATE", 409, "No se puede firmar: no hay corrida de QA registrada."
                )
            fallos = await todos(
                db,
                """SELECT criterio_id FROM core.qa_results
            ...
            bloqueantes = await todos(
                db,
                """SELECT id FROM core.security_findings
            ...
            if bloqueantes:
                raise AppError(
                    "PRECONDICION_GATE",
                    409,
                    "No se puede firmar: hay hallazgos de seguridad bloqueantes sin resolver.",
                )
        ...
        sello = await registrar_evento(
            ...
            usuario.tenant_id,
            evento="gate_firmado",
            resumen=f"Gate {gate} aprobado",
            actor_tipo="humano",
            actor_id=usuario.user_id,
            ticket_id=numero,
            datos={
                "gate": gate,
                "alternativa": datos.alternativa_id,
        ...
        try:
            await db.execute(
                ...
                VALUES (:i, :t, :tk, :g, 'aprobado', :u, :c, :alt, :af, :sp, :se)"""),
                {
                    ...
                    "u": usuario.user_id,
                    "c": datos.comentario,
                    "alt": datos.alternativa_id,
                    "af": datos.acepta_fallos,
                    "sp": spec and spec["id"],
                    "se": sello,
                },
            )
            ...
            raise AppError(
                "GATE_NO_ACTIVO",
                409,
                f"Este gate ya fue firmado o no cumple precondiciones ({type(e).__name__}).",
            )
        # ----- reanuda el job (T7/T8/T9 sincronizan línea, historial y ticket) --
        destino = _GATE_AVANZA_A[gate]
        await db.execute(
            text("UPDATE core.jobs SET estado=:e WHERE id=:j"), {"e": destino, "j": job["id"]}
        )
        ...
        if gate == 1 and spec and spec["estimacion_loc"] and cuota["limite"]:
            if spec["estimacion_loc"] > cuota["limite"] - cuota["consumidas"]:
                aviso = (
                    "La estimación de LOC de esta spec supera la cuota restante "
...
@router.post("/tickets/{numero}/gates/{gate}/request-changes", summary="Pedir cambios en un gate")
async def pedir_cambios(
    numero: str,
    gate: int,
    datos: CambiosIn,
    usuario: UsuarioActual = Depends(requiere_rol(*_ROLES_APROBADOR)),
    ...
    async with sesion_de(usuario) as db:
        ...
        await db.execute(
            text("""UPDATE core.jobs SET estado=:e,
        ...
        if escalado:
            await db.execute(
                        ...
                        CAST(:d AS jsonb))"""),
                {
                    ...
                    "j": job["id"],
                    "e": destino,
                    "d": _json.dumps({"gate": gate, "iteracion": n}),
                },
            )
    log.info("gates.cambios_pedidos", ticket_id=numero, gate=gate, iteracion=n, escalado=escalado)
    if escalado:
        raise AppError(
            "MAX_ITERACIONES",
            409,
