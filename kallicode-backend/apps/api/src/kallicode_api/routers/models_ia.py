...
from ..deps import UsuarioActual, requiere_rol
...
async def providers(
    usuario: UsuarioActual = Depends(requiere_rol("owner", "admin", "architect")),
) -> dict:
    ...
    return {
        ...
        "tiers": [
            {"tier": t.value, "modelo": m, "nombre": _TIER_INFO[t][0], "perfil": _TIER_INFO[t][1]}
            for t, m in (
                (Tier.FLASH, s.llm_flash_model),
...
async def registrar_llave(
    proveedor: str, usuario: UsuarioActual = Depends(requiere_rol("owner", "admin"))
) -> dict:
    """En modalidad gestionada (D10) el tenant no aporta llaves: 403."""
    raise funcion_no_disponible("En tu modalidad las llaves de modelos las gestiona Kallicode.")
...
async def assignments(
    ...
    pasos = [
        {
            ...
            "etapa": pid.split(".")[0],
            "tier": pl["tier"].value,
            "modelo": modelo[pl["tier"]],
            "escalable": pl["escalable"],
    ...
    return {
        "politica_escalado": "flash → pro → fable (fallo de esquema x2, "
