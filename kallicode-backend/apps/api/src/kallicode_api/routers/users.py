...
class UserPatch(BaseModel):
    rol: str | None = Field(
        default=None, pattern=r"^(owner|admin|architect|approver|member|viewer)$"
    )
...
async def patch_me(datos: MePatch, usuario: UsuarioActual = Depends(usuario_actual)) -> dict:
    ...
    async with sesion_de(usuario) as db:
        ...
        if datos.password_nueva:
            if not datos.password_actual or not verificar_password(
                datos.password_actual, u["password_hash"]
            ):
                raise AppError(
                    "PASSWORD_ACTUAL_INCORRECTA", 400, "La contraseña actual no es correcta."
                )
            validar_politica_password(datos.password_nueva)
            await db.execute(
                text("UPDATE core.users SET password_hash=:p WHERE id=:i"),
                {"p": hashear_password(datos.password_nueva), "i": u["id"]},
            )
            await db.execute(
                text("UPDATE core.refresh_tokens SET revocado=true WHERE user_id=:i"),
                {"i": u["id"]},
            )
            await registrar_evento(
                db,
                usuario.tenant_id,
                evento="usuario_modificado",
                resumen="Contraseña cambiada",
                actor_tipo="humano",
                actor_id=u["id"],
            )
        import json
        ...
        for campo in ("nombre", "idioma"):
            ...
            if v is not None:
                await db.execute(
                    text(f"UPDATE core.users SET {campo}=:v WHERE id=:i"), {"v": v, "i": u["id"]}
                )
        if datos.notificaciones is not None:
            await db.execute(
                text(
                    "UPDATE core.users SET notificaciones = "
...
async def listar(
    ...
    usuario: UsuarioActual = Depends(requiere_rol("owner", "admin", "architect", "approver")),
) -> dict:
    ...
    if q:
        cond.append("(nombre ILIKE :q OR email ILIKE :q)")
        params["q"] = f"%{q}%"
    where = " AND ".join(cond)
    async with sesion_de(usuario) as db:
        items = await todos(
            db,
            f"""SELECT id, nombre, email, rol, estado, ultimo_acceso
                                     FROM core.users WHERE {where}
                                     ORDER BY nombre LIMIT :lim OFFSET :off""",
            {**params, "lim": page_size, "off": (page - 1) * page_size},
        )
        total = (await uno(db, f"SELECT count(*) AS n FROM core.users WHERE {where}", params))["n"]
    return {"items": items, "total": total, "page": page, "page_size": page_size}


@router.post("/invitations", status_code=201, summary="Invitar usuario")
async def invitar(
    datos: InviteIn, usuario: UsuarioActual = Depends(requiere_rol("owner", "admin"))
) -> dict:
    ...
    if datos.rol in ("owner", "admin") and usuario.rol != "owner":
        raise AppError(
            "ROL_NO_AUTORIZADO", 403, "Solo el propietario puede invitar administradores."
        )
    ...
    async with sesion_de(usuario) as db:
        ...
        if dup:
            raise AppError(
                "INVITACION_DUPLICADA", 409, "Ya hay una invitación pendiente para este correo."
            )
...
async def patch_user(
    user_id: str, datos: UserPatch, usuario: UsuarioActual = Depends(requiere_rol("owner", "admin"))
) -> dict:
    ...
    async with sesion_de(usuario) as db:
        ...
        if datos.rol and datos.rol != "owner" and u["rol"] == "owner":
            ...
            if owners <= 1:
                raise AppError(
                    "ULTIMO_OWNER", 409, "La organización debe tener al menos un propietario."
                )
        if (datos.rol and u["rol"] == "architect" and datos.rol != "architect") or (
            ...
            if arqs == 0:
                raise AppError(
                    "ULTIMO_ARQUITECTO",
                    409,
                    "No puedes quitar el rol: es el único usuario que puede firmar el Gate 3.",
                )
        ...
        for campo, v in cambios.items():
            await db.execute(
                text(f"UPDATE core.users SET {campo}=:v WHERE id=:i"), {"v": v, "i": user_id}
            )
