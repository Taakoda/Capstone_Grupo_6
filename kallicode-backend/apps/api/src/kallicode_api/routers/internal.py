...
TRANSICIONES: dict[str, set[str]] = {
    ...
    "escalado_humano": {"triage", "design", "build", "qa", "security", "deploy", "cancelado"},
}
...
async def _idempotente(db, svc: ServicioActual, clave: str, endpoint: str):
    ...
    fila = await uno(
        db,
        """SELECT respuesta, status_code FROM core.idempotency_keys
...
async def _memorizar(
    db, svc: ServicioActual, clave: str, endpoint: str, respuesta: dict, status: int = 200
) -> None:
    await db.execute(
        text("""
        ...
        {"c": clave, "t": svc.tenant_id, "e": endpoint, "r": json.dumps(respuesta), "s": status},
    )
...
class EscalationIn(BaseModel):
    ...
    paso: str | None = None
    motivo: str = Field(
        pattern=r"^(validacion_persistente|baja_confianza|max_iteraciones|"
...
async def _ticket(db, tenant_id: str, numero: str) -> dict:
    t = await uno(
        db,
        "SELECT * FROM core.tickets WHERE tenant_id=:t AND numero=:n",
...
async def _job_activo(db, job_id: str) -> dict:
    j = await uno(
        db,
        """SELECT * FROM core.jobs WHERE id=:i
...
async def crear_job(
    ...
    async with sesion_de(svc) as db:
        ...
        org = await uno(
            db, "SELECT fabrica_activa FROM core.organizations WHERE id=:t", {"t": svc.tenant_id}
        )
        ...
        existente = await uno(
            db,
            """SELECT id, estado FROM core.jobs WHERE ticket_id=:tk
...
async def asignar(
    ...
    async with sesion_de(svc) as db:
        ...
        if memo:
            return memo["respuesta"]
        await _job_activo(db, job_id)
        linea = await uno(
            db,
            """SELECT * FROM core.production_lines
        ...
        try:
            await db.execute(
                text("UPDATE core.jobs SET linea=:n, estado='triage' WHERE id=:j"),
                {"n": datos.linea, "j": job_id},
            )
        except Exception:
            raise conflicto("LINEAS_EXCEDIDAS", "El plan no permite más líneas concurrentes.")
...
async def transicion(
    ...
    async with sesion_de(svc) as db:
        ...
        if (
            de in _GATES
            and datos.a in TRANSICIONES.get(de, set())
            and datos.a not in ("cancelado",)
        ):
            raise conflicto(
                "GATE_REQUERIDO", f"La transición requiere la firma humana del gate {_GATES[de]}."
            )
        if datos.a not in TRANSICIONES.get(de, set()):
            raise conflicto("TRANSICION_INVALIDA", f"Transición {de}→{datos.a} no permitida.")
        ...
        if datos.a == "gate1":
            sp = await uno(
                db,
                """SELECT id FROM core.specs WHERE ticket_id=:tk
                                   AND estado='pendiente_gate1'""",
                {"tk": job["ticket_id"]},
            )
            if not sp:
                raise conflicto("ARTEFACTO_FALTANTE", "Falta la spec para entrar a gate1.")
        if datos.a == "gate2":
            qa = await uno(
                db,
                "SELECT id FROM core.qa_runs WHERE ticket_id=:tk LIMIT 1",
                {"tk": job["ticket_id"]},
            )
            if not qa:
                raise conflicto("ARTEFACTO_FALTANTE", "Falta la corrida de QA para entrar a gate2.")
        ...
        await db.execute(
            text("""UPDATE core.jobs SET estado=:a,
            ...
            {
                "a": datos.a,
                "it": json.dumps(iteraciones),
                "cx": json.dumps(datos.contexto) if datos.contexto else None,
                "j": job_id,
            },
        )
        numero = (
            await uno(db, "SELECT numero FROM core.tickets WHERE id=:i", {"i": job["ticket_id"]})
        )["numero"]
        await registrar_evento(
            db,
            svc.tenant_id,
            evento="job_transicion",
            resumen=f"{de} → {datos.a}" + (f" ({datos.motivo})" if datos.motivo else ""),
            actor_tipo="agente",
            actor_id=svc.servicio,
            ticket_id=numero,
            job_id=job_id,
            etapa=datos.a,
        )
...
async def registrar_paso(
    ...
    async with sesion_de(svc) as db:
        ...
        estado_norm = {
            ...
            "pendiente_asignacion": "triage",
        }.get(job["estado"], job["estado"])
        if datos.etapa != estado_norm and job["estado"] != "escalado_humano":
            raise conflicto(
                "ETAPA_INCONSISTENTE", f"El job está en {job['estado']}, no en {datos.etapa}."
            )
        sid = nuevo("st")
        numero = (
            await uno(db, "SELECT numero FROM core.tickets WHERE id=:i", {"i": job["ticket_id"]})
        ...
        await db.execute(
                    ...
                    :cf, :va, :re, :tin, :tout, :du, :er, :sr)"""),
            {
                ...
                "re": datos.reintentos,
                "tin": datos.tokens_in,
                "tout": datos.tokens_out,
                "du": datos.duracion_ms,
                "er": datos.entrada_ref,
                "sr": datos.salida_ref,
            },
        )
        await db.execute(
...
async def triage_result(
    ...
    async with sesion_de(svc) as db:
        ...
        await db.execute(
             ...
             WHERE id=:i"""),
            {
                ...
                "p": datos.prioridad,
                "dup": dup_id,
                "imp": json.dumps(datos.impacto),
                "cx": datos.complejidad,
                "i": t["id"],
            },
        )
        siguiente = "cerrado_duplicado" if dup_id else "design"
        await db.execute(
            text("""UPDATE core.jobs SET estado=:e WHERE ticket_id=:tk
                                  AND estado NOT IN ('produccion','cancelado',
                                                     'cerrado_duplicado')"""),
            {"e": siguiente, "tk": t["id"]},
        )
    ...
    log.info(
        "triage.resultado", ticket_id=numero, prioridad=datos.prioridad, duplicado=bool(dup_id)
    )
    return {"ticket_id": numero, "siguiente": siguiente}


@router.post(
    "/tickets/{numero}/spec-versions", status_code=201, summary="Registrar versión de spec"
)
async def spec_version(
    numero: str,
    datos: SpecIn,
    svc: ServicioActual = Depends(servicio_actual),
    idempotency_key: str = Header(alias="Idempotency-Key"),
    ...
    if len(rec) != 1:
        raise AppError(
            "VALIDACION_ENTRADA", 422, "Debe haber exactamente una alternativa recomendada."
        )
    if any(p.startswith(("infra/secrets", "/")) for p in datos.scope_paths):
        raise AppError(
            "SCOPE_PROHIBIDO", 422, "scope_paths incluye rutas vetadas por la política del tenant."
        )
    async with sesion_de(svc) as db:
        t = await _ticket(db, svc.tenant_id, numero)
        version = (
        ...
        await db.execute(
            text("""UPDATE core.specs SET estado='reemplazada'
        ...
        await db.execute(
                    ...
                    CAST(:adr AS jsonb), :loc)"""),
            {
                ...
                "tk": t["id"],
                "v": version,
                "md": datos.spec_md_ref,
                "sc": datos.scope_paths,
        ...
        for c in datos.criterios:
            await db.execute(
                ...
                VALUES (:s, :t, :c, :g, :w, :th)"""),
                {"s": sid, "t": svc.tenant_id, "c": c.id, "g": c.given, "w": c.when, "th": c.then},
            )
        await db.execute(
            text("""UPDATE core.jobs SET estado='gate1' WHERE ticket_id=:tk
                                  AND estado NOT IN ('produccion','cancelado',
                                                     'cerrado_duplicado')"""),
            {"tk": t["id"]},
        )
        await registrar_evento(
            db,
            svc.tenant_id,
            evento="spec_generada",
            etapa="design",
            resumen=f"Spec v{version} generada con {len(datos.alternativas)} alternativas",
            actor_tipo="agente",
            actor_id="design",
            ticket_id=numero,
        )
        # notificación de gate pendiente a aprobadores
        aprobadores = await todos(
            db,
        ...
        for u in aprobadores:
            await db.execute(
                ...
                VALUES (:i, :t, :u, 'gate_pendiente', :ti, :c, :tk)"""),
                {
                    ...
                    "c": f"Spec v{version} lista para tu aprobación.",
                    "tk": t["id"],
                },
            )
...
@router.post("/tickets/{numero}/evidence", status_code=201, summary="Registrar corrida de QA")
async def evidencia(
    numero: str,
    datos: EvidenciaIn,
    svc: ServicioActual = Depends(servicio_actual),
    ...
    async with sesion_de(svc) as db:
        ...
        spec = await uno(
            db,
            """SELECT * FROM core.specs WHERE ticket_id=:tk
        ...
        criterios = {
            c["criterio_id"]
            for c in await todos(
                db, "SELECT criterio_id FROM core.spec_criteria WHERE spec_id=:s", {"s": spec["id"]}
            )
        }
        ...
        if faltantes:
            raise AppError(
                "MATRIZ_INCOMPLETA", 422, f"Faltan criterios en la matriz: {sorted(faltantes)}."
            )
        rid = nuevo("qr")
        await db.execute(
            text("""
...
@router.post(
    "/tickets/{numero}/findings", status_code=201, summary="Registrar veredicto de Security"
)
async def findings(
    numero: str,
    datos: FindingsIn,
    ...
    if (datos.veredicto_cambio == "blocked") != bool(bloqueantes):
        raise AppError(
            "VEREDICTO_INCONSISTENTE", 422, "blocked sin hallazgos bloqueantes (o viceversa)."
        )
    async with sesion_de(svc) as db:
        t = await _ticket(db, svc.tenant_id, numero)
        for h in datos.hallazgos:
            ...
            await registrar_evento(
                db,
                svc.tenant_id,
                evento="hallazgo_seguridad",
                etapa="security",
                resumen=f"{h.titulo} ({h.veredicto})",
                actor_tipo="agente",
                actor_id="security",
                ticket_id=numero,
                datos={"severidad": h.severidad_contextual, "justificacion": h.justificacion},
            )
    ...
    log.info(
        "security.veredicto",
        ticket_id=numero,
        veredicto=datos.veredicto_cambio,
...
async def preflight(
    ...
    async with sesion_de(svc) as db:
        ...
        firmas = {
            f["gate"]
            for f in await todos(
                db,
                """SELECT gate FROM core.gate_signatures
                    WHERE ticket_id=:tk AND accion='aprobado'""",
                {"tk": t["id"]},
            )
        }
        bloqueantes = (
            await uno(
                db,
                """SELECT count(*) AS n FROM core.security_findings
...
async def deploy_result(
    numero: str,
    datos: DeployResultIn,
    svc: ServicioActual = Depends(servicio_actual),
    idempotency_key: str = Header(alias="Idempotency-Key"),
) -> dict:
    ...
    async with sesion_de(svc) as db:
        ...
        if datos.resultado == "exito" and datos.destino == "produccion":
            await db.execute(
                text("""UPDATE core.jobs SET estado='produccion'
                                      WHERE ticket_id=:tk AND estado NOT IN
                                            ('produccion','cancelado','cerrado_duplicado')"""),
                {"tk": t["id"]},
            )
            watchers = await todos(
                db,
                """SELECT user_id FROM core.ticket_watchers
                                           WHERE ticket_id=:tk""",
                {"tk": t["id"]},
            )
            for w in watchers:
        ...
        await registrar_evento(
            ...
            actor_id="deploy",
            ticket_id=numero,
            datos={"post_deploy": datos.post_deploy, "anomalia": datos.anomalia},
        )
        job = await uno(
            db,
            """SELECT linea FROM core.jobs WHERE ticket_id=:tk
                                ORDER BY creado_en DESC LIMIT 1""",
            {"tk": t["id"]},
        )
    if datos.resultado == "anomalia":
        log.warning(
            "deploy.anomalia", ticket_id=numero, recomendacion=datos.anomalia.get("recomendacion")
        )
    log.info("deploy.resultado", ticket_id=numero, destino=datos.destino, resultado=datos.resultado)
    return {
        "ticket": {
            "id": numero,
            "etapa": "produccion"
...
async def registrar_loc(
    ...
    async with sesion_de(svc) as db:
        ...
        if cruzado:
            ...
            msj = (
                "Alcanzaste el 80% de la cuota mensual de LOC."
                if cruzado == "aviso_80"
                else "Cuota mensual de LOC agotada: los tickets nuevos quedan en cola."
            )
            for o in owners:
                await db.execute(
                    text("""
            ...
            log.warning(
                f"usage.{'umbral_80' if cruzado == 'aviso_80' else 'agotado'}",
                consumidas=cuota["consumidas"],
                limite=cuota["limite"],
            )
    log.info("usage.loc", ticket_id=datos.ticket_id, netas=datos.netas, consumo=cuota["consumidas"])
    return {
        "consumo_ciclo": {
            "consumidas": cuota["consumidas"],
            "limite": cuota["limite"],
...
async def escalar(
    ...
    async with sesion_de(svc) as db:
        ...
        existente = await uno(
            db,
            """SELECT id FROM core.escalations
        ...
        await db.execute(
            text("UPDATE core.jobs SET estado='escalado_humano' WHERE id=:j"), {"j": datos.job_id}
        )
        numero = (
            await uno(db, "SELECT numero FROM core.tickets WHERE id=:i", {"i": job["ticket_id"]})
        )["numero"]
    ...
    nivel(
        "escalations.abierta", job_id=datos.job_id, motivo=datos.motivo, severidad=datos.severidad
    )
