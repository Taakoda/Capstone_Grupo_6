...
async def _org(db, tenant_id: str) -> dict:
    ...
    return {
        ...
        and {
            ...
            "nombre": plan["nombre"],
            "lineas": plan["lineas"],
            "loc_mes": plan["loc_mes"],
            "renueva_el": str(plan["renueva_el"]),
        },
        "polling": {"notificaciones_s": s.poll_notificaciones_s, "tablero_s": s.poll_tablero_s},
    }
...
async def actualizar(
    datos: OrgPatch, usuario: UsuarioActual = Depends(requiere_rol("owner", "admin"))
) -> dict:
    ...
    async with sesion_de(usuario) as db:
        for campo, v in cambios.items():
            await db.execute(
                text(f"UPDATE core.organizations SET {campo} = :v WHERE id = :t"),
        ...
        if cambios:
            await registrar_evento(
                db,
                usuario.tenant_id,
                evento="organizacion_modificada",
                resumen="Organización actualizada",
                actor_tipo="humano",
                actor_id=usuario.user_id,
                datos={"campos": list(cambios)},
            )
