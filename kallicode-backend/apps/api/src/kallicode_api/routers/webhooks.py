...
async def _recibir(
    ...
    clave = secreto.get("webhook_secret") or secreto.get("api_token") or secreto.get("token") or ""
    ...
    if not clave or not hmac.compare_digest(esperada, recibida):
        log.warning("webhooks.firma_invalida", proveedor=proveedor, connection_id=cn["id"])
        raise AppError("FIRMA_INVALIDA", 401, "")
    ...
    if fila is None:
        ...
        log.info(
            f"webhooks.{proveedor}.recibido",
            delivery_id=delivery_id,
            evento=evento,
            connection_id=cn["id"],
        )
        await comercial.redis_cliente().xadd("kc:inbox", {"inbox_id": str(fila["id"])})
    return {"recibido": True, "tenant_id": cn["tenant_id"], "inbox_id": fila and fila["id"]}
...
async def jira(
    ...
    r = await _recibir(
        "jira", token, request, x_hub_signature, x_atlassian_webhook_identifier, "jira:issue"
    )
...
async def github(
    ...
    r = await _recibir(
        "github", token, request, x_hub_signature_256, x_github_delivery, x_github_event
    )
    return {"recibido": r["recibido"]}


@router.post("/kallicode-help/{token}", status_code=202, summary="Webhook de Kallicode Help")
async def kallicode_help(
    token: str,
    request: Request,
    response: Response,
    x_kc_signature: str | None = Header(default=None),
    x_kc_delivery: str | None = Header(default=None),
) -> dict:
    ...
    r = await _recibir("help", token, request, x_kc_signature, x_kc_delivery, "help:ticket")
    async with sesion_sistema() as db:
        plan = await uno(
            db,
            """SELECT p.permite_help FROM core.subscriptions s
    ...
    if plan and not plan["permite_help"]:
        raise AppError(
            "FUNCION_NO_DISPONIBLE", 403, "Kallicode Help no está incluido en el plan del tenant."
        )
...
async def corestream(
    token: str,
    request: Request,
    x_cs_signature: str | None = Header(default=None),
    x_cs_delivery: str | None = Header(default=None),
) -> dict:
    r = await _recibir("corestream", token, request, x_cs_signature, x_cs_delivery, "cs:ticket")
    return {"recibido": r["recibido"]}
...
async def ci(
    ...
    r = await _recibir("ci", token, request, x_hub_signature_256, x_github_delivery, "ci:resultado")
    return {"recibido": r["recibido"]}
