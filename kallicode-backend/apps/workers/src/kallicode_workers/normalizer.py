...
def _de_jira(p: dict) -> dict | None:
    ...
    prioridad = {
        ...
        "low": "baja",
        "lowest": "baja",
    }.get(str((campos.get("priority") or {}).get("name", "")).lower(), "media")
    return {
        "tipo": "bug",
        "titulo": campos.get("summary") or issue.get("key", "Jira"),
        "descripcion": json.dumps(campos.get("description") or "")[:10000] or "-",
        "prioridad": prioridad,
        "clave_externa": issue.get("key"),
    }
...
def _de_github(p: dict) -> dict | None:
    ...
    return {
        "tipo": tipo,
        "titulo": issue.get("title", "GitHub issue"),
        "descripcion": (issue.get("body") or "-")[:10000],
        "prioridad": "media",
        "clave_externa": str(issue.get("number")),
    }
...
def _de_help(p: dict) -> dict | None:
    ...
    return {
        "tipo": tipo,
        "titulo": p.get("titulo", "Kallicode Help"),
        "descripcion": (p.get("descripcion") or "-")[:10000],
        "prioridad": "alta" if p.get("diagnostico", {}).get("confianza", 0) > 0.8 else "media",
...
def _de_corestream(p: dict) -> dict | None:
    ...
    return {
        "tipo": tipo,
        "titulo": tk.get("title", "Corestream"),
...
_NORMALIZADORES = {
    "jira:issue": _de_jira,
    "issues": _de_github,
    "help:ticket": _de_help,
    "cs:ticket": _de_corestream,
    "ci:resultado": None,
}
...
async def procesar_inbox(campos: dict) -> None:
    ...
    async with sesion_sistema() as db:
        fila = await uno(
            db,
            "SELECT * FROM core.webhook_inbox WHERE id=:i "
        ...
        if fila["evento"] == "ci:resultado":
            ...
            await db.execute(
                text(
                    "UPDATE core.webhook_inbox SET estado='procesado', "
        ...
        if ticket is None:
            ...
            await db.execute(
                text("UPDATE core.webhook_inbox SET estado=:e, procesado_en=now() WHERE id=:i"),
                {"e": estado, "i": inbox_id},
            )
            return
    ...
    async with sesion_tenant(tenant, actor="svc:normalizer") as db:
        origen = {
            "jira:issue": "jira",
            "issues": "github",
            "help:ticket": "kallicode_help",
            "cs:ticket": "corestream",
        }[fila["evento"]]
        ...
        creado = await uno(
            ...
            RETURNING numero""",
            {
                ...
                "oref": json.dumps(
                    {
                        "clave_externa": ticket.get("clave_externa"),
                        "diagnostico": ticket.get("diagnostico"),
                    }
                ),
                "uf": json.dumps(ticket.get("usuario_final"))
                if ticket.get("usuario_final")
                else None,
            },
        )
        await db.execute(
            text("""UPDATE core.webhook_inbox SET estado='procesado',
        ...
        await registrar_evento(
            ...
            actor_tipo="sistema",
            ticket_id=creado["numero"],
        )
    if not pausado:
        await redis_cliente().xadd(
            "kc:triage", {"ticket_id": creado["numero"], "tenant_id": tenant}
        )
    log.info(
        "normalizer.ticket",
        inbox_id=inbox_id,
...
async def procesar_triage(campos: dict) -> None:
    ...
    try:
        async with httpx.AsyncClient(timeout=15) as cli:
            r = await cli.post(
                f"{s.embeddings_url}/v1/embeddings",
                json={"model": "bge-m3", "input": f"{t['titulo']}\n{t['descripcion'][:2000]}"},
            )
            r.raise_for_status()
    ...
    try:
        ...
        resultado = await ejecutar_paso(
            "triage.classify",
            {
                "ticket_id": numero,
                "titulo": t["titulo"],
                "origen": t["origen"],
                "descripcion": t["descripcion"][:4000],
                "adjuntos_transcritos": [],
            },
        )
        async with sesion_tenant(tenant, actor="svc:triage") as db:
            await registrar_evento(
        ...
        log.warning("triage.llm_no_disponible", ticket_id=numero, error=type(e).__name__)
