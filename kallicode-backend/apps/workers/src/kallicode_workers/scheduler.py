        ...
        async with sesion_tenant(tenant, actor="svc:scheduler") as db:
            ...
            linea = await uno(
                ...
                SELECT numero FROM core.production_lines
                 ...
                 ORDER BY numero FOR UPDATE SKIP LOCKED LIMIT 1""",
                {"t": tenant},
            )
            if not linea:
                return  # QU-2: sin línea libre, el job espera
            await db.execute(
                text("""UPDATE core.jobs SET linea=:n, estado='triage'
                                      WHERE id=:j"""),
            ...
            await registrar_evento(
                ...
                actor_tipo="sistema",
                ticket_id=job["ticket_numero"],
                job_id=job["id"],
            )
        log.info("scheduler.asignado", job_id=job["id"], linea=linea["numero"], tenant_id=tenant)
